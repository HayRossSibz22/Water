...
public class Buoyancy : MonoBehaviour {
    ...
    public float rotationAmount = 2f; // Degree of rotation per key press
    public float pushForce = 0.5f; // Force applied when rowing
    public float momentumDecay = 0.98f;

            public float rotationSpeed = 30f; // Speed at which the boat will rotate over time
...
private float lastPressTimeS = -1f;
private float pressCooldown = 1f; // One second cooldown
...
private void Update() {
    ...
    bool isAPressed = Input.GetKey(KeyCode.A) && Time.time - lastPressTimeA > pressCooldown;
    bool isSPressed = Input.GetKey(KeyCode.S) && Time.time - lastPressTimeS > pressCooldown;

    // Check if both A and S keys are being pressed for straight movement
    if (isAPressed && isSPressed) {
        ApplyForce(pushForce * 0.5f); // Apply a smaller force for straight movement
        lastPressTimeA = Time.time;
        lastPressTimeS = Time.time;
    } else {
        // Individual key presses with cooldown
        if (isAPressed) {
            lastPressTimeA = Time.time;
            targetYRotation -= rotationAmount;
            ApplyForce(pushForce * 0.5f); // Apply half force for rotation
        }
        if (isSPressed) {
            lastPressTimeS = Time.time;
            targetYRotation += rotationAmount;
    ...
    transform.rotation = Quaternion.Euler(0, currentYRotation + 90, 0);

    // Apply momentum decay
    currentVelocity *= momentumDecay;

    // Move the boat using the current velocity
    transform.Translate(currentVelocity * Time.deltaTime, Space.World);
}

private void ApplyForce(float force) {
    // Apply force in the object's local forward direction, taking into account the initial rotation offset
    var localForward = Quaternion.Euler(0, 90, 0) * transform.forward;
    currentVelocity += localForward * force;
}
